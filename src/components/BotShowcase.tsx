...
export function BotShowcase() {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-3 border-b border-foreground/10 bg-foreground/[0.035] px-5 py-4">
        <div className="font-display flex h-10 w-10 items-center justify-center rounded-full bg-accent font-bold text-background">
          D
        </div>
        <div>
            ...
            <div key={message} className="relative min-h-14">
              {!reduceMotion && (
                <m.div
                  ...
                  className="absolute flex w-14 items-center justify-center gap-1 rounded-xl rounded-tl-sm bg-foreground/[0.08] px-3 py-3"
                >
                  {[0, 1, 2].map((dot) => (
                    <m.span
                      key={dot}
                      ...
                      className="h-1.5 w-1.5 rounded-full bg-foreground/55"
                    />
                  ))}
                </m.div>
              )}
              <m.div
                ...
                className="max-w-[92%] rounded-2xl rounded-tl-sm border border-foreground/[0.06] bg-foreground/[0.075] px-4 py-3 text-[0.78rem] leading-relaxed text-foreground/80 shadow-sm sm:text-sm"
              >
                {message}
                <span className="ml-2 whitespace-nowrap font-mono text-[0.56rem] text-foreground/30">{time}</span>
              </m.div>
            </div>
          );
        })}
